function [store_B,store_z,store_phi,store_q,store_R2,store_gam,store_s2,y,x,u,T,k] = SpikeSlabGLP_Lap8(y,x,u,abeta,bbeta,Abeta,Bbeta,M,N,lambda)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Linear regression with a spike-and-slab prior and Laplace likelihood
%
% Inputs:
...
%   lambda: scale parameter of the Laplace likelihood
%
% Outputs:
...
lambda = 1;  % Default value for Laplace scale parameter
...
store_B(:, 1) = b0;
b = b0;
s2 = sum(abs(resy - x * b)) / T;  % Adjusted for Laplace likelihood
store_s2(1) = s2;
z = (b ~= 0);
store_z(:, 1) = z;
tau = sum(z);
...
for i = 2:M
    ...
    for j = 1:k
        ...
        % Log-posterior under Laplace likelihood
        log_pr_z0 = tau0 * log(q) + (k - tau0) * log(1 - q) - tau0 * log(gam) - ...
            .5 * 2 * sum(log(diag(chol(W0)))) - lambda * sum(abs(y - u * phi - x(:, logical(z0)) * bhat0));

        log_pr_z1 = tau1 * log(q) + (k - tau1) * log(1 - q) - tau1 * log(gam) - ...
            .5 * 2 * sum(log(diag(chol(W1)))) - lambda * sum(abs(y - u * phi - x(:, logical(z1)) * bhat1));
    ...
    % Draw s2 (residual variance, adjusted for Laplace)
    niu = sum(abs(y - u * phi - x(:, logical(z)) * bhat));  % Laplace residual
    s2 = 1 / gamrnd(T, 1 / (niu / lambda));
    store_s2(i) = s2;
    
    % Draw b (regression coefficients)
